; Load generated overworld palettes, background characters, and area-dependent
; sprite characters. Area records select forced-blank loads, active-display
; queues, directional transition schedules, animation tracks, and special-scene
; asset phases. VRAM layout ownership remains in overworld_vram.asm.

lorom

incsrc "symbols.inc"

!free_space_bank_a0_start = $A0A620
!free_space_bank_a0_end = $A0AC00
!free_space_bank_82_start = $82F5CC
...
hook_WorldMapOverworldRestore:
    ...
    JSR.w !SelectAndClearOverworldVRAM
    JSR SynchronousLoadCurrentOverworldAssets
    RTL
...
; Load the area record selected by A and synchronously process every batch
...
;
; Direct-page scratch:
;   $00-$02: area-record pointer, then current batch pointer
...
SynchronousLoadOverworldAssetsStart:
    ...
    JSR ResolveOverworldAreaRecord
    JSR.w !ActivateOverworldAnimationTracks

    ; $03:$05 <- 24-bit pointer to full-load batch sequence
    LDY.w #$0000                ; Start at the full-load pointer in the record.
    LDA.b [$00],Y               ; Read it through the area-record pointer.
...
; Resolve the area record for key A and the current game state. Each pointer-
